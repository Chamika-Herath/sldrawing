...
class main_user_login_ADD_UPDATE
{
    ...
    private $type_of_access_creating_custome_state = false;

    public function set_registration_from_data_for_customer(
        $get_user_name,
        $get_password,
        $get_name_show
    ) {
        ...
        $main_user_account_access_level_list_LIST_obj->filter_by_type_of_access("customer");
        $get_result = $main_user_account_access_level_list_LIST_obj->get_result();


        if ($get_result && $get_result->num_rows > 0) {
            while ($row = $get_result->fetch_assoc()) {
                $this->type_of_access_creating_custome_state = true;
                $this->main_user_account_access_level_list_id = $row["id"];
            }
        }



        $this->last_name = $get_name_show;
        $this->first_name = $get_name_show;
    ...
    public function  get_type_of_access_creating_custome_state()
    {
        return $this->type_of_access_creating_custome_state;
    }
