...
include_once '../../../Controllers/Customer-Supplier/cus_sup_list/cus_sup_list_ADD_UPDATE.php';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($get_result && $get_result->num_rows == 0) {
        ...
        if (isset($_POST['main_user_login_create_for_customer'])) {
            $main_user_login_ADD_UPDATE_obj->set_registration_from_data_for_customer($user_name, $password, $name_show);
            if ($main_user_login_ADD_UPDATE_obj->get_type_of_access_creating_custome_state()) {
            } else {
                $state['error'] = "1";
                $state['message'] = "Not Access Level for Customer";
                $json[] = $state;
        ...
        if (isset($_POST['id'])) {
            ...
            if ($main_user_login_ADD_UPDATE_obj->process_new_record()) {
                ...
                if (isset($_POST['main_user_login_create_for_customer'])) {
                    $cus_sup_list_ADD_UPDATE_obj = new cus_sup_list_ADD_UPDATE($main_user_login_ADD_UPDATE_obj->get_id());

                    $cus_sup_list_ADD_UPDATE_obj->set_data($name_show, $name_show, $user_name, "", "");
                    $cus_sup_list_ADD_UPDATE_obj->is_active();
                    $cus_sup_list_ADD_UPDATE_obj->is_self_account();
                    $cus_sup_list_ADD_UPDATE_obj->is_personal_account();

                    if ($cus_sup_list_ADD_UPDATE_obj->process_create_new_record_for_customer()) {

                        $state['error'] = "0";
                        $state['message'] = "cus_sup_create";
                        $state['id'] = $cus_sup_list_ADD_UPDATE_obj->get_id();
                    } else {

                        $state['error'] = "1";
                        $state['message'] = $cus_sup_list_ADD_UPDATE_obj->get_error_message();
                    }
                }
            } else {
                $state['error'] = $main_user_login_ADD_UPDATE_obj->get_error();
