...
include_once '../../../Controllers/Customer-Supplier/cus_sup_list/cus_sup_list_SIGNLE_DATA_by_main_user_login.php';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['id'])) {
        ...
        if(isset($_POST['cus_sup_list_details_update'])) {

          $cus_sup_list_SIGNLE_DATA_by_main_user_login_obj = new cus_sup_list_SIGNLE_DATA_by_main_user_login($user_id); 

          if($cus_sup_list_SIGNLE_DATA_by_main_user_login_obj->get_bool_state()){

          $cus_sup_list_id = $cus_sup_list_SIGNLE_DATA_by_main_user_login_obj->get_id();

          }else{
            $state['error'] = '1';
            $state['message'] = 'cus sup details not found';
            $json[] = $state;
            echo json_encode($json);
            exit; 
          }
          ...
          $cus_sup_list_ADD_UPDATE_obj = new cus_sup_list_ADD_UPDATE($user_id);
          
            $cus_sup_list_ADD_UPDATE_obj->set_id($cus_sup_list_id);
            $cus_sup_list_ADD_UPDATE_obj->set_phone_no($phone_number);  
            if($cus_sup_list_ADD_UPDATE_obj->process_update_record()){

            }else{
                $state['error'] = '1';
                $state['message'] = 'cus sup update error';
            }
